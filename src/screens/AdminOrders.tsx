  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 overflow-y-auto bg-surface">
      ...
      <div className="p-4 lg:p-8 max-w-5xl mx-auto w-full space-y-5">
        {/* Status summary cards — horizontal scroll on mobile, grid on desktop */}
        <div className="flex gap-2 overflow-x-auto pb-1 lg:grid lg:grid-cols-7 lg:overflow-visible" style={{ scrollbarWidth: "none" }}>
          <button
            onClick={() => setFilter("all")}
            className={`text-left rounded-xl border p-3 transition-all shrink-0 min-w-[72px] lg:min-w-0 ${filter === "all" ? "border-primary bg-surface-container-low ring-1 ring-primary/20" : "border-outline-variant/20 bg-surface-container-lowest hover:border-outline-variant/40"}`}
          >
            <span className="text-[9px] font-bold text-on-surface-variant uppercase tracking-wider">All</span>
            <p className="font-bold text-primary font-mono" style={{ fontSize: 22, lineHeight: 1.1 }}>{orders.length}</p>
          </button>
          {allStatuses.map((s) => (
            <button
              key={s}
              onClick={() => setFilter(filter === s ? "all" : s)}
              className={`text-left rounded-xl border p-3 transition-all shrink-0 min-w-[72px] lg:min-w-0 ${filter === s ? "border-primary bg-surface-container-low ring-1 ring-primary/20" : "border-outline-variant/20 bg-surface-container-lowest hover:border-outline-variant/40"}`}
            >
              <span className="text-[9px] font-bold text-on-surface-variant uppercase tracking-wider whitespace-nowrap">
                {s.charAt(0).toUpperCase() + s.slice(1)}
              </span>
              <p className={`font-bold font-mono ${counts[s] > 0 && (s === "pending" || s === "confirmed" || s === "baking") ? "text-primary" : "text-on-surface-variant"}`} style={{ fontSize: 22, lineHeight: 1.1 }}>
                {counts[s]}
              </p>
            </button>
          ))}
